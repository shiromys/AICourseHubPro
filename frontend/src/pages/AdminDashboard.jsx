    ...
    <div className="flex min-h-screen bg-gray-900 font-sans text-gray-100 relative">
            ...
            <div className="bg-gray-800 p-6 rounded-xl max-w-sm w-full border border-gray-700">
                ...
                <div className="mb-4">
                    ...
                    <input 
                        ...
                        className="w-full bg-gray-900 border border-gray-700 p-2 rounded mt-1 text-white"
                    />
                </div>
      ...
      <div className="flex-1 ml-64 p-8 bg-gray-900 min-h-screen">
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 hover:border-green-500/50 transition-all group">
                  <div className="flex justify-between items-start mb-4"><div className="p-3 rounded-lg bg-green-500/10 text-green-500 group-hover:bg-green-500 group-hover:text-white transition"><DollarSign size={24}/></div></div>
                  <h3 className="text-3xl font-black text-white">${stats.revenue}</h3>
                  <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mt-1">Total Revenue</p>
              </div>
              <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 hover:border-blue-500/50 transition-all group">
                  <div className="flex justify-between items-start mb-4"><div className="p-3 rounded-lg bg-blue-500/10 text-blue-500 group-hover:bg-blue-500 group-hover:text-white transition"><Users size={24}/></div></div>
                  <h3 className="text-3xl font-black text-white">{stats.students}</h3>
                  <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mt-1">Total Students</p>
              </div>
              <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 hover:border-purple-500/50 transition-all group">
                  <div className="flex justify-between items-start mb-4"><div className="p-3 rounded-lg bg-purple-500/10 text-purple-500 group-hover:bg-purple-500 group-hover:text-white transition"><BookOpen size={24}/></div></div>
                  <h3 className="text-3xl font-black text-white">{stats.courses}</h3>
                  <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mt-1">Total Courses</p>
              </div>
              <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 hover:border-orange-500/50 transition-all group">
                  <div className="flex justify-between items-start mb-4"><div className="p-3 rounded-lg bg-orange-500/10 text-orange-500 group-hover:bg-orange-500 group-hover:text-white transition"><Activity size={24}/></div></div>
                  <h3 className="text-3xl font-black text-white">{stats.uptime}</h3>
                  <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mt-1">System Uptime</p>
              </div>
            </div>
            ...
            <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-lg">
                <div className="p-6 border-b border-gray-700 flex justify-between items-center bg-gray-800/50">
                    <div className="relative w-96">
                        ...
                        <input type="text" placeholder="Search courses..." className="w-full pl-10 pr-4 py-2.5 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-red-500 transition" />
                    </div>
                    <button onClick={handleOpenModal} className="flex items-center gap-2 px-5 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 transition font-bold shadow-lg shadow-red-900/30"><Plus size={18} /> Add New Course</button>
                </div>
                <table className="w-full text-left">
                    <thead className="bg-gray-900 text-gray-400 text-xs uppercase font-bold tracking-wider"><tr><th className="px-6 py-4">Title</th><th className="px-6 py-4">Price</th><th className="px-6 py-4">Category</th><th className="px-6 py-4">Modules</th><th className="px-6 py-4 text-right">Actions</th></tr></thead>
                    <tbody className="divide-y divide-gray-700">
                        {courses.length === 0 ? <tr><td colSpan="5" className="text-center py-12 text-gray-500 italic">No courses available.</td></tr> : courses.map((course) => (
                            <tr key={course.id} className="hover:bg-gray-700/30 transition">
                                <td className="px-6 py-4 font-medium text-white">{course.title}</td>
                                <td className="px-6 py-4 text-green-400 font-bold">${course.price}</td>
        ...
        {activeTab === 'users' && (
            <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-lg">
                <div className="p-6 border-b border-gray-700 flex justify-between items-center bg-gray-800/50">
                    <h3 className="font-bold text-lg text-white">Active User Directory</h3>
                    <span className="bg-green-500/20 text-green-400 text-xs font-bold px-3 py-1 rounded-full">{users.length} Users</span>
                </div>
                <table className="w-full text-left">
                    <thead className="bg-gray-900 text-gray-400 text-xs uppercase font-bold tracking-wider"><tr><th className="px-6 py-4">User</th><th className="px-6 py-4">Role</th><th className="px-6 py-4">Status</th><th className="px-6 py-4 text-right">Actions</th></tr></thead>
                    <tbody className="divide-y divide-gray-700 text-sm">
                        {users.map(user => (
                            <tr key={user.id} className="hover:bg-gray-700/30 transition">
                                <td className="px-6 py-4"><div className="font-bold text-white">{user.name}</div><div className="text-xs text-gray-500">{user.email}</div></td>
        ...
        {activeTab === 'deleted_users' && (
            <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
                <div className="p-4 bg-red-900/20 text-red-300 text-sm border-b border-red-900/30 flex items-center gap-2 font-bold"><Trash2 size={16}/> These accounts are deactivated. Restore them to grant access again.</div>
                <table className="w-full text-left">
                    <thead className="bg-gray-900 text-gray-400 text-xs uppercase font-bold"><tr><th className="px-6 py-4">User</th><th className="px-6 py-4">Email</th><th className="px-6 py-4 text-right">Actions</th></tr></thead>
                    <tbody className="divide-y divide-gray-700 text-sm">
                        {users.length === 0 ? <tr><td colSpan="3" className="p-8 text-center text-gray-500 italic">No deleted users found.</td></tr> : users.map(user => (
                            <tr key={user.id} className="hover:bg-gray-700/30 opacity-75">
                                <td className="px-6 py-4 line-through text-gray-500 font-medium">{user.name}</td>
                                <td className="px-6 py-4 text-gray-500">{user.email}</td>
                                <td className="px-6 py-4 text-right"><button onClick={() => handleRestoreUser(user)} className="text-green-400 hover:text-green-300 flex items-center gap-2 ml-auto bg-green-900/20 px-3 py-1.5 rounded transition font-bold"><RefreshCcw size={16}/> Restore</button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
        ...
        {activeTab === 'revenue' && (
            <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-lg">
                <div className="p-6 border-b border-gray-700 flex justify-between items-center">
                    ...
                    <button onClick={handleExportCSV} className="text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded transition">Export CSV</button>
                </div>
                <table className="w-full text-left">
                    <thead className="bg-gray-900 text-gray-400 text-xs uppercase font-bold tracking-wider"><tr><th className="px-6 py-4">User</th><th className="px-6 py-4">Course</th><th className="px-6 py-4">Date</th><th className="px-6 py-4">Status</th></tr></thead>
                    <tbody className="divide-y divide-gray-700 text-sm">
                        {transactions.length === 0 ? <tr><td colSpan="4" className="p-12 text-center text-gray-500 italic">No transactions found in database.</td></tr> : transactions.map((tx, i) => (
                            <tr key={i} className="hover:bg-gray-700/30 transition">
                                <td className="px-6 py-4 font-bold text-white">{tx.user}<div className="text-xs text-gray-500 font-normal">{tx.email}</div></td>
                                <td className="px-6 py-4 text-gray-300">{tx.course}</td>
                                <td className="px-6 py-4 text-gray-500 font-mono text-xs">{tx.date}</td>
                                <td className="px-6 py-4"><span className="text-green-400 text-xs border border-green-900 bg-green-900/20 px-2 py-1 rounded font-bold flex items-center gap-1 w-fit"><CheckCircle size={12}/> Paid</span></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
        ...
        {/* SUPPORT TAB */}
        {activeTab === 'support' && (
            <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-lg h-[80vh] flex flex-col">
                <div className="p-6 border-b border-gray-700 bg-gray-800/50 flex justify-between items-center">
                    <h3 className="font-bold text-lg flex items-center gap-2 text-white"><Mail className="text-blue-500"/> Support Inbox</h3>
                    <span className="bg-blue-500/20 text-blue-400 text-xs font-bold px-2 py-1 rounded">{messages.length} Messages</span>
                </div>
                <div className="divide-y divide-gray-700 overflow-y-auto flex-1 custom-scrollbar">
                    {messages.length === 0 ? <div className="p-20 text-center text-gray-500 italic">No support messages received yet.</div> : messages.map(msg => (
                        <div key={msg.id} className={`p-6 transition cursor-pointer group ${msg.is_read ? 'opacity-50 hover:opacity-80' : 'hover:bg-gray-700/30'}`}>
                            <div className="flex justify-between mb-2">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-white group-hover:text-blue-400 transition flex items-center gap-2">{msg.subject}{!msg.is_read && <span className="bg-red-500 text-white text-[10px] px-2 rounded-full">New</span>}</h4>
                                        <p className="text-xs text-gray-400">From: <span className="text-white">{msg.name}</span> &lt;{msg.email}&gt;</p>
                                    </div>
                                </div>
                                <span className="text-xs text-gray-500 font-mono">{msg.date}</span>
                            </div>
                            <div className="pl-13 ml-13 mt-3 bg-gray-900/50 p-4 rounded-lg border border-gray-700/50"><p className="text-sm text-gray-300 leading-relaxed">{msg.message}</p></div>
                            <div className="mt-3 flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition">
                                {!msg.is_read && (<button onClick={() => initiateCloseTicket(msg.id)} className="text-xs bg-gray-700 text-white px-3 py-1 rounded hover:bg-gray-600 flex items-center gap-1 shadow-md"><CheckSquare size={12}/> Close Ticket</button>)}
                            </div>
                        </div>
                    ))}
                </div>
        ...
        {activeTab === 'audit' && (
            <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-lg">
                <div className="p-6 border-b border-gray-700 bg-gray-800/50">
                    <h3 className="font-bold text-lg flex items-center gap-2 text-white"><FileText className="text-orange-500"/> System Audit Logs</h3>
                    <p className="text-xs text-gray-500 mt-1">Tracking all administrative actions for security compliance.</p>
                </div>
                <table className="w-full text-left">
                    <thead className="bg-gray-900 text-gray-400 text-xs uppercase font-bold tracking-wider"><tr><th className="px-6 py-4">Action</th><th className="px-6 py-4">Admin</th><th className="px-6 py-4">Details</th><th className="px-6 py-4">Time</th></tr></thead>
                    <tbody className="divide-y divide-gray-700 text-sm">
                        {logs.map((log, i) => (
                            <tr key={i} className="hover:bg-gray-700/30 transition">
                                <td className="px-6 py-4 font-bold text-white flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-orange-500"></div>{log.action}</td>
                                <td className="px-6 py-4 text-gray-400 font-mono text-xs">{log.admin}</td>
            ...
            <div className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
                        <h3 className="font-bold mb-6 flex items-center gap-2 text-red-400"><Activity size={20} /> Live System Status</h3>
                        <div className="space-y-4">
                            <div className="flex justify-between items-center p-4 bg-gray-900/50 rounded-lg border border-gray-700/50"><span className="text-gray-200 font-medium">Database Connection</span><span className="text-green-400 flex items-center gap-2 text-sm font-bold bg-green-500/10 px-3 py-1 rounded-full border border-green-500/20"><CheckCircle size={14}/> Connected</span></div>
                            <div className="flex justify-between items-center p-4 bg-gray-900/50 rounded-lg border border-gray-700/50"><span className="text-gray-200 font-medium">API Response Time</span><span className="text-green-400 text-sm font-bold font-mono">24ms</span></div>
                            <div className="flex justify-between items-center p-4 bg-gray-900/50 rounded-lg border border-gray-700/50"><span className="text-gray-200 font-medium">Stripe Webhooks</span><span className="text-green-400 text-sm font-bold flex items-center gap-2"><div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div> Listening</span></div>
                            <div className="flex justify-between items-center p-4 bg-gray-900/50 rounded-lg border border-gray-700/50"><span className="text-gray-200 font-medium">Storage Usage</span><div className="w-32 h-2 bg-gray-700 rounded-full overflow-hidden"><div className="h-full bg-blue-500 w-[45%]"></div></div></div>
                        </div>
                    </div>
                    <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg flex flex-col">
                        <h3 className="font-bold mb-4 text-white">Server Load (Real-time)</h3>
                        <div className="h-64 flex-1">
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={systemLoadData}>
                                    <Line type="monotone" dataKey="load" stroke="#ef4444" strokeWidth={3} dot={false} isAnimationActive={true} />
        ...
        {/* SETTINGS TAB */}
        {activeTab === 'settings' && (
            <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 max-w-3xl shadow-lg">
                <h3 className="font-bold mb-8 text-2xl text-white flex items-center gap-3"><Settings className="text-gray-400"/> Global Configurations</h3>
                <div className="space-y-6">
                    <div className="flex items-center justify-between p-6 bg-gray-900 rounded-xl border border-gray-700">
                        <div><h4 className="font-bold text-gray-200 text-lg">Maintenance Mode</h4><p className="text-sm text-gray-500 mt-1">If enabled, non-admin users will see a "Under Maintenance" page.</p></div>
                        <button onClick={() => setSettings(p => ({...p, maintenance: !p.maintenance}))} className={`w-14 h-8 rounded-full p-1 transition-colors duration-300 ${settings.maintenance ? 'bg-red-600' : 'bg-gray-700'}`}><div className={`w-6 h-6 bg-white rounded-full transform transition-transform duration-300 shadow-md ${settings.maintenance ? 'translate-x-6' : 'translate-x-0'}`} /></button>
                    </div>
                    <div className="flex items-center justify-between p-6 bg-gray-900 rounded-xl border border-gray-700">
                        <div><h4 className="font-bold text-gray-200 text-lg">Allow New Registrations</h4><p className="text-sm text-gray-500 mt-1">Toggle to pause new user signups temporarily.</p></div>
                        <button onClick={() => setSettings(p => ({...p, registrations: !p.registrations}))} className={`w-14 h-8 rounded-full p-1 transition-colors duration-300 ${settings.registrations ? 'bg-green-600' : 'bg-gray-700'}`}><div className={`w-6 h-6 bg-white rounded-full transform transition-transform duration-300 shadow-md ${settings.registrations ? 'translate-x-6' : 'translate-x-0'}`} /></button>
                    </div>
                    <div className="p-6 bg-red-900/10 rounded-xl border border-red-900/30 mt-8">
                        <h4 className="font-bold text-red-500 mb-2 flex items-center gap-2"><AlertTriangle size={18}/> Danger Zone</h4>
                        <p className="text-gray-400 text-sm mb-4">Irreversible actions for system management.</p>
                        <div className="flex gap-4">
                            <button className="px-4 py-2 border border-red-800 text-red-500 rounded hover:bg-red-900/20 text-sm font-bold transition">Clear Cache</button>
                            <button className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 text-sm font-bold transition shadow-lg shadow-red-900/20">Reset Database (Dev Only)</button>
                        </div>
                    </div>
                </div>
            </div>
